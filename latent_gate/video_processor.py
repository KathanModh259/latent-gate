...
class VideoProcessor:
    ...
    def __init__(self, config: Optional[PipelineConfig] = None, video_config: Optional[VideoConfig] = None):
    ...
    def extract_frames(
        ...
        start_frame = int(self.video_config.start_time * video_fps) if self.video_config.start_time > 0 else 0
        end_frame = int(self.video_config.end_time * video_fps) if self.video_config.end_time else total_frames
        ...
        while cap.isOpened() and frame_count < (end_frame - start_frame):
            ...
            if frame_count % frame_interval == 0:
                ...
                cv2.imwrite(
                    path,
                    frame,
                    [cv2.IMWRITE_JPEG_QUALITY, self.video_config.quality]
                )
    ...
    def _generate_summary(self, results: List[dict]) -> dict:
        ...
        return {
            ...
            "average_tokens": sum(r["tokens_estimated"] for r in decoded_results) / max(len(decoded_results), 1),
        }
